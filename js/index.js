...
function FindTheCode(digits, combinations_amount, combinations_type, code){
    ...
    let i = 0, listOfTries = [], map = [], index_map = [], newCode = ""

    console.log('limit', limit)

    while(newCode != code && i <= limit){
        if(i == 0){
            for(let n = 0; n <= digits - 1; n++){
                map[n] = 0 
            }
        }
        ...
        for(let k = 0; k <= digits - 1; k++){
            map[k] = (combinations_type == 'text') ? lettersLow[index_map[k]] : numbers[index_map[k]]
        }
        newCode = map.join('')
        listOfTries.push(newCode)
    ...
    return `listOfTries.length: ${listOfTries.length} || code ${code}`
}
